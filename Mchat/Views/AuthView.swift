...
struct AuthView: View {
    ...
    private func handleSignIn() async {
        ...
        do {
            // Get all tokens from Amplify and set them in AuthManager
            let tokens = try await AuthService.getAllTokens()
            await MainActor.run {
                auth.setAuthenticatedFromAmplify(
                    idToken: tokens.idToken,
                    accessToken: tokens.accessToken,
                    refreshToken: tokens.refreshToken
                )
            }
            // Post-sign-in bootstrap is centralized in AuthManager; nothing else to do here
        } catch {
    ...
    private func handleConfirm() async {
        ...
        do {
            // 3) Get all tokens from Amplify and set them in AuthManager
            let tokens = try await AuthService.getAllTokens()
            await MainActor.run {
                auth.setAuthenticatedFromAmplify(
                    idToken: tokens.idToken,
    ...
    // Bootstrap logic is centralized in AuthManager.postSignInBootstrap()
}
